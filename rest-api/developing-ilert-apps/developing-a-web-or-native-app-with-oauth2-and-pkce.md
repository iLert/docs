...
Before redirecting the user to the iLert authorization server, we need to generate a verified and code challenge that can be used by the authorization server to ensure the origin of the later token request matches the origin of the authorization request.

{% hint style="info" %}
PKCE might seem confusing at first, but in the end it is just a random identifier that verifies the one who initially started the authorization request, is the same who comes asking for the token. And to ensure noone else knows the plain version of the identifier, it is hashed before sharing it with the authorization server.
{% endhint %}

Below are some helper methods to deal with PKCE generation:
...
{% hint style="success" %}
In case the user is known to your application or especially if the application is limited to a single account. We suggest to redirect the user directly to the **tenant** (account ulr) of the user, this way he might be able to skip the login step if logged in already. e.g. `https://someTenant.ilert.com/api/developers/oauth2/authorize` instead of `https://app.ilert.com/api/developers/...`
{% endhint %}
...
If the user has accepted your scope grant, iLert's authorization server will redirect the user back to your provided **redirect\_uri** while adding a **code** query parameter to it. This code parameter can be used in the next step to fetch the access token.
...
After a successfull redirect, we can detect the code query parameter and run an AJAX call to fetch our tokens.

{% hint style="info" %}
You may use plain XMLHttpRequest or the new fetch API, for the sake of simplicty in reading we chose the library **axios** to make the HTTP calls.
{% endhint %}
...
You will receive an erro response.
...
{% hint style="danger" %}
Never expose your user's tokens, especially not the refresh tokens, keep them safe like passwords.
{% endhint %}
...
As access tokens are short lived, you will have to refresh them regularly, at least in case they expire and the iLert API returns a 401 HTTP status code error.
